...
class TestLRMATrainingEquations:
    ...
    def test_critic_mse_loss_equation(self):
        ...
        s = torch.randn(4, 236, device=device)
        a = torch.randn(4, 55, device=device)
        y = torch.tensor([[14.9], [12.0], [8.5], [20.1]], device=device)
    ...
    def test_actor_gradient_direction_increases_q(self):
        # Deterministic seed for gradient direction verification
        torch.manual_seed(42)
        actor = EDActor(9, 2)
        ...
        optimizer = torch.optim.Adam(actor.parameters(), lr=0.05)

        s = torch.randn(1, 9, device=device)
        a_prob = actor(s)
